...
async def get_db_session_with_context(
    ...
    if user_id:
        try:
            ...
            logger.error(
                f"Failed to set RLS context for user {user_id} in QualA worker: {e}",
                exc_info=True,
    ...
    if rls_set_success or not user_id:
        try:
            ...
            logger.error(
                f"Database error during session for user {user_id} in QualA worker: {e}",
                exc_info=True,
                extra={"props": {"user_id": str(user_id)}},
            )
            await db.rollback()
            ...
            logger.error(
                f"Unexpected error during session for user {user_id} in QualA worker: {e}",
                exc_info=True,
                extra={"props": {"user_id": str(user_id)}},
            )
            await db.rollback()
            ...
            if rls_set_success:
                try:
                    ...
                    logger.warning(
                        f"Failed to reset RLS context for user {user_id} in QualA worker: {e}",
                        extra={"props": {"user_id": str(user_id)}},
                    )
            await db.close()
            current_user_id_cv.set(None)
